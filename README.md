...
- 工夫点
  ...
  - お酒専門の投稿サイトなので、お酒以外の不適切な画像や投稿内容が上がることへの対処を考え、お問い合わせ機能の導入と管理者にて不適切な投稿の強制削除、あるいは強制退会が出来るよう機能を加えました。
  - メールに不具合が生じ、万が一お問い合わせメールが届かなくなることまで想定し、管理者のみが閲覧することのできるお問い合わせ内容一覧ページを作成しました。
    また、管理者自身の誤った退会を防ぐために、管理者のページのみ自主的な退会ボタン及びユーザ強制退会ボタンを非表示にしました。
  - 退会を論理削除にした上、退会しているユーザかどうかをユーザ詳細画面から判別できるようにしました。
  - 検索機能のドロップダウン初期設定を完全一致ではなく部分一致にすることで、絞り込み検索以外の場合にドロップダウンを変更しなければならないという余計な手間を省きつつ、検索機能としての汎用性を高めました。
  - ログインしていない場合にフォローボタンやDMボタン、Createボタンを押下した際、ログインページに遷移されるように設定しました。
  - ユーザ検索の際、管理者が非表示になるよう設定しました。
  - エラーメッセージを日本語化しました。
  - 基本アクション以外のアクション名や、class名などの説明を短く・分かりやすいように記述することを意識し、コメントで補足しました。
  - レスポンシブ対応にしました。
- 苦労点
  - テーブルと改行の投稿に伴う表示のレスポンシブ対応について。
  - 縦画像と横画像の投稿で、投稿一覧ページ上での全体的なレイアウトのバランスを保つ表示について。
  - ダイレクトメッセージの通知機能の実装。
